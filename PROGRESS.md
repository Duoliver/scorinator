...
| 4 | Standings calc (configurable points, live update) | `engine/standings` | ✅ Done | 2026-09-04 | 2026-09-04 | [004](/progress-reports/004-standings.md) | Tie-break: points, GD, GF, head-to-head, roster order. Adds `sortOrder`/`position`/`positionText` — see report. |
...
- **2026-09-04 (Task 4, second follow-up):** The user asked for tied league positions to display correctly. `StandingsRow` gained three fields: `sortOrder` (the row's place in the table, always unique), `position` (the standings rank, shared by rows level on every footballing criterion, including head-to-head), and `positionText` (`position` as a string on the first row of a tied block, `'-'` on the rest). Proposed `sortOrder` as a name for the first field, since the user asked for a better one than "order"; open to a different name. Flagged, not fixed: a 3-or-more-way head-to-head result cycle could in theory make the tied-block grouping inconsistent, since the comparator would no longer be transitive for that group. See `progress-reports/004-standings.md` for the full reasoning.
