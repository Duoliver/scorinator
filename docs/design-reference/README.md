...
**Why this matters:** without this separation, a task like "build the team creation form" tends to become "read the whole handoff bundle and build a fully styled, fully wired screen in one pass" — which bypasses TDD for the UI layer entirely and tends to reintroduce coupling between domain logic and presentation that the module boundaries in §1 are meant to prevent.
