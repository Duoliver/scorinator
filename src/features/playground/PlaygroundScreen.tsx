...
function Section({
  ...
  return (
    <section class={styles.section}>
      <h2>{title}</h2>
      <div class={styles.sectionRow}>{children}</div>
    </section>
  );
}
...
function PointsPreview({
  ...
  return <span class={styles.muted}>Currently: {display}</span>;
}
 ...
 * existed. It stays as a permanent, dev-only diagnostic screen — see
 * PROGRESS.md Task 22 — rather than a deleted one, since it is still the
 * only manual click-through path for the real save/open dialog (Task 21's
 * report, and PERSISTENCE.md), which no automated test in this sandbox can
 * cover. `AppShell` gates its nav item and route behind `import.meta.env.DEV`. */
export function PlaygroundScreen(): JSX.Element {
  ...
  return (
    <div class={styles.screen}>
      <h1>Design system + engine playground</h1>
      ...
      <div class={styles.body}>
        ...
        <Section title="Input">
          ...
          <span class={styles.muted}>{loggedValues ?? ''}</span>
        </Section>
        ...
        <Section title="Card">
          <Card padding="md">
            ...
            <div class={styles.muted}>Round robin (two-way) · 8 teams</div>
          </Card>
        </Section>
